...
class BeastProgressService {
  ...
  static List<ProgressCategory> buildCategories({
    ...
    return [
      _buildSkillsCategory(state: state, config: config),
      _buildTalentsCategory(state: state, config: config),
      _buildSkinsCategory(state: state, config: config),
    ];
  }
  ...
  static double calculateSkillsProgress({
    ...
    return _buildSkillsCategory(state: state, config: config).progress;
  }
  ...
  static double calculateTalentsProgress({
    ...
    return _buildTalentsCategory(state: state, config: config).progress;
  }
  ...
  static double calculateSkinsProgress({
    ...
    return _buildSkinsCategory(state: state, config: config).progress;
  }
  ...
  static ProgressCategory _buildSkillsCategory({
    ...
    final items = beastSkillDefinitions.map((skill) {
      ...
      final currentLevel = state.skillLevels[skill.id] ?? skill.minLevel;
      ...
      return ProgressItem(
        ...
        targetValue: (itemConfig?.targetLevel ?? skill.maxLevel).toDouble(),
  ...
  static ProgressCategory _buildTalentsCategory({
    ...
    final talentConfigs = config.talentConfigsFor(state.selectedBeast);

    final items = talents.map((talent) {
      final itemConfig = talentConfigs[talent.id];

      final currentLevel = state.talentLevels[talent.id] ?? 0;
      ...
      return ProgressItem(
        ...
        currentValue: currentLevel.toDouble(),
        minimumValue: 0,
        targetValue: (itemConfig?.targetLevel ?? talent.maxLevel).toDouble(),
        isTracked: itemConfig?.isTracked ?? true,
        weight: 1,
      );
    }).toList();
  ...
  static ProgressCategory _buildSkinsCategory({
    ...
    final items = beastSkinDefinitions.map((skin) {
      ...
      final currentLevel = state.skinLevels[skin.id] ?? skin.minLevel;
      ...
      return ProgressItem(
        ...
        targetValue: (itemConfig?.targetLevel ?? skin.maxLevel).toDouble(),
        isTracked: itemConfig?.isTracked ?? true,
        weight: 1,
      );
    }).toList();
